class Quotation
  ...
  def self.get_quotation(mood)
    @mood=mood
    if mood
      mood = all_options[mood].sample
      @message= mood[0]
      @image= make_meme_request
    end
    Quotation.new(@message, @image)
  end

  def self.make_meme_request
    @url="http://version1.api.memegenerator.net/Generators_Search?q=#{@mood}&pageIndex=0&pageSize=12"
